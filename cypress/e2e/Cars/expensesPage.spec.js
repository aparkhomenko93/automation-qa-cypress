...
describe.only("Check adding expenses to a created car", () => {
    //Test user
    const user = {
    ...
    //test expense data
    const expensesData = {
        litersQty: faker.number.int({min: 1, max: 60}),
        cost: faker.number.int({min: 1, max: 500})
    }
    ...
    it("Add expense to existing car", () => {
        garagePage.addCar(car);
        garagePage.checkCarCreated(car);

        cy.visit('panel/expenses');
        expensesPage.addFuelExpense(car, expensesData);
        expensesPage.checkExpensesCreated(car, expensesData);
    })
})